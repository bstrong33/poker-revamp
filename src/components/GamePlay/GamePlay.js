...
function GamePlay(props) {
    ...
    // useEffect(() => {
    //     setSocket({socket: io.connect("http://localhost:3333")})
    // }, [])
    
    useEffect(() => {
        // let { username, id, initialMoney } = props;
        // console.log("useEffect", username, id, initialMoney)
        // let startMoney = parseInt(initialMoney);
        socket.socket.on('room_joined', data => {
            joinSuccess(data)
        })
        // socket.emit('join game', { username, id, startMoney })
        socket.socket.on('dealt_out', viewCards)
        socket.socket.on('preflop_betting', allowPreflopBetting)
        socket.socket.on('no_money_left', noMoneyLeft)
        socket.socket.on('flop', setFlop)
        socket.socket.on('turn', setTurn)
        socket.socket.on('river', setRiver)
        socket.socket.on('winners', displayWinners)
    }, [socket.socket])
    ...
    function joinRoom () {
        ...
        if (state.room) {
            ...
            // console.log('emmited to join room', state.room)

        //Adding in
        let { username, id, initialMoney } = props;
        // console.log(username, id, initialMoney)
        let startMoney = parseInt(initialMoney);
        socket.socket.emit('join_game', { username, id, startMoney })
        // Added above
    ...
    function joinSuccess (data) {
        ...
        // console.log("joinSuccess ran, the room is", state.room)
    }
    ...
    function readyToPlay() {
        ...
        socket.socket.emit('ready', { room: state.room })
        // console.log("readyToPlay ran the room is", state.room)
    }

    // recieiving cards from server and setting state to be able to display these cards
    function viewCards(players) {
        // console.log("VIEW CARDS", players)
        let otherPlayers = []
        let { id } = props
    ...
    function allowPreflopBetting(data) {
        // console.log("data sent to allowPreflopBetting", data)
        ...
        // console.log("PREFLOP BETTING", data)
    }
    ...
    function displayPreflopBetting() {
        console.log('display preflop betting', state)
        socket.socket.emit('turn_of_preflop_betting', {
            room: state.room, player: state.player
        })
    }

    function displayflopBetting() {
        // console.log('display flop betting', state)
        socket.socket.emit('flop_betting', {
            room: state.room
        })
        // console.log("just ran displayFlopBetting")
    }

    function displayTurnBetting() {
        // console.log('display turn betting', state)
        socket.socket.emit('turn_betting', {
            room: state.room
        })
    }

    function displayRiverBetting() {
        // console.log('display river betting', state)
        socket.socket.emit('river_betting', {
            room: state.room
        })
    }
    ...
    function callBet() {
        ...
        let sortedOtherPlayers = [...state.otherPlayers]
    ...
    function displayWinners(data) {
        let { winnerNames, potTotal, players, roomName } = data
        // let { winnerNames, potTotal } = data
        // console.log("display winner", winnerNames)
        ...
        viewingWinners(players, roomName);
    }

    function viewingWinners(players, roomName) {
        // console.log("viewing winners", state)
        setTimeout(() => {
            setState(prevState => ({
                ...prevState,
                winnerNames: null,
            ...
            startNextHand(players, roomName);
        }, 30000)
    }

    function startNextHand(players, roomName) {
        console.log("start next hand", players)
        console.log("props", props)
        console.log("state", state)
        if (players[1].id === props.id) {
            console.log('ran');
            socket.socket.emit('start next hand', {
                room: roomName
            })
        }
    }
    ...
    function showCards(player) {
        let { cards } = state
        // console.log(player, cards[0].name)
        let cardOne = null
        let cardTwo = null
